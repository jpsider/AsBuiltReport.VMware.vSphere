...
Set-Location "C:\OPEN_PROJECTS\AsBuiltReport.VMware.vSphere\"
...
foreach ($Directory in $Directories)
{
    ...
    $files = Get-ChildItem .\AsBuiltReport.VMware.vSphere\$Directory
    foreach ($file in $files)
    {
        # Source the file
        $FileName = $file.Name
        Write-Output "Importing file $FileName"
        . .\AsBuiltReport.VMware.vSphere\$Directory\$FileName
    }
    # Execute Pester for the Directory.
    $results = Invoke-pester .\tests\$Directory\*.ps1 -CodeCoverage .\AsBuiltReport.VMware.vSphere\$Directory\*.ps1 -PassThru -ExcludeTag SkipThis
